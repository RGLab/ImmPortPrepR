...
data types, and controlled terms by using `checkTemplate()`.  This function takes in the
data frame we have created as well as the template name.  Any problems will throw an error,
so for demonstration purposes we wrap the function in a `tryCatch()` method here.

```{r check-template}
# NOTE: messages are printed out
results <- tryCatch(checkTemplate(df = study,
                                  ImmPortTemplateName = "study"),
